...
func _draw_field() -> void:
	...
	field_bg.color = Constants.COLOR_FIELD
	...
	var circle_size = Constants.CELL_SIZE * 3
	center.color = Constants.COLOR_CENTER_CIRCLE
	center.size = Vector2(circle_size, circle_size)
	center.position = Vector2(
		Constants.FIELD_CENTER_X * Constants.CELL_SIZE + Constants.CELL_SIZE/2 - circle_size/2,
		Constants.FIELD_CENTER_Y * Constants.CELL_SIZE + Constants.CELL_SIZE/2 - circle_size/2
	)
	center.z_index = -5
	add_child(center)
	...
	left_goal.color = Constants.COLOR_GOAL
	left_goal.size = Vector2(Constants.CELL_SIZE, (Constants.GOAL_Y_MAX - Constants.GOAL_Y_MIN + 1) * Constants.CELL_SIZE)
	left_goal.position = Vector2(0, Constants.GOAL_Y_MIN * Constants.CELL_SIZE)
	left_goal.z_index = -5
	add_child(left_goal)

	var right_goal = ColorRect.new()
	right_goal.color = Constants.COLOR_GOAL
	right_goal.size = Vector2(Constants.CELL_SIZE, (Constants.GOAL_Y_MAX - Constants.GOAL_Y_MIN + 1) * Constants.CELL_SIZE)
	right_goal.position = Vector2((Constants.GRID_WIDTH - 1) * Constants.CELL_SIZE, Constants.GOAL_Y_MIN * Constants.CELL_SIZE)
...
func _draw_zone_line(x_cell: int) -> void:
	...
	line.color = Constants.COLOR_ZONE_LINE
	line.size = Vector2(2, Constants.FIELD_PIXEL_HEIGHT)
	line.position = Vector2((x_cell + 1) * Constants.CELL_SIZE - 1, 0)
	line.z_index = -5
	add_child(line)
...
func _create_unit_visuals() -> void:
	for unit in home_team.units:
		_create_unit_visual(unit, Constants.COLOR_HOME_TEAM)
	for unit in away_team.units:
		_create_unit_visual(unit, Constants.COLOR_AWAY_TEAM)


func _create_unit_visual(unit: Unit, base_color: Color) -> void:
	var visual = ColorRect.new()
	visual.size = Vector2(Constants.UNIT_SIZE, Constants.UNIT_SIZE)
	...
	label.size = Vector2(Constants.UNIT_SIZE, Constants.UNIT_SIZE)
...
func _create_ball_visual() -> void:
	...
	ball_visual.size = Vector2(Constants.BALL_SIZE, Constants.BALL_SIZE)
	ball_visual.color = Constants.COLOR_BALL
...
func _update_all_visuals() -> void:
	for unit in home_team.units:
		_update_unit_visual(unit, Constants.COLOR_HOME_TEAM, Constants.COLOR_HOME_BALL)
	for unit in away_team.units:
		_update_unit_visual(unit, Constants.COLOR_AWAY_TEAM, Constants.COLOR_AWAY_BALL)
	_update_ball_visual()
...
func _update_unit_visual(unit: Unit, base_color: Color, ball_color: Color) -> void:
	...
	visual.position = Vector2(
		pixel_pos.x + (Constants.CELL_SIZE - Constants.UNIT_SIZE) / 2,
		pixel_pos.y + (Constants.CELL_SIZE - Constants.UNIT_SIZE) / 2
	)
...
func _update_ball_visual() -> void:
	...
	ball_visual.position = Vector2(
		pixel_pos.x - Constants.BALL_SIZE / 2,
		pixel_pos.y - Constants.BALL_SIZE / 2
	)
